...
object Anagrams {
  ...
  def toOccurences(list: List[(Char, Int)]): Occurrences = {
    list
  ...
  def wordOccurrences(w: Word): Occurrences =
    toOccurences(w.groupBy(_.toLower).mapValues(_.size).toList)
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    val emptyMapOfLists = Map[Occurrences, List[Word]]() withDefaultValue (List())
    dictionary.foldLeft(emptyMapOfLists)({ case (mapSoFar, newWord) =>
      val occurrences: Occurrences = wordOccurrences(newWord)
      mapSoFar.updated(occurrences, newWord :: mapSoFar(occurrences))
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      ...
      else combinations(occurrences).flatMap { word: Occurrences =>
        val possibleWords: List[Word] = dictionaryByOccurrences(word)
        (for {
          w: Word <- possibleWords
          s: Sentence <- sentenceAnagrams(subtract(occurrences, word))
        } yield w :: s).toList
      }
    }

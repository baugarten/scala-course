...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    if (bits.isEmpty) Nil
      ...
      val (char, newBits) = decodeBit(tree, bits)
      char :: decode(tree, newBits)
    }
  }

  def decodeBit(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
    tree match {
      case Fork(left, right, chars, weight) => bits.head match {
        case 0 => decodeBit(left, bits.tail)
        case 1 => decodeBit(right, bits.tail)
      }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    if (text.isEmpty) Nil
    else bitStringForChar(tree, text.head) ++ encode(tree)(text.tail)
  }

  def bitStringForChar(tree: CodeTree, char: Char): List[Bit] = {
    tree match {
      case Fork(left, right, _, weight) =>
        if (chars(left).contains(char)) 0 :: bitStringForChar(left, char)
        else 1 :: bitStringForChar(right, char)
  ...
  def convert(tree: CodeTree): CodeTable = {
    chars(tree).map(c => (c, bitStringForChar(tree, c)))
  }

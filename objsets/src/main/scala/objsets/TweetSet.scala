...
object NilTweet extends Tweet("", "", -1)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(other: TweetSet): TweetSet = {
    new NonEmpty(
      elem,
      other.filterAcc(_.text < elem.text, left),
      other.filterAcc(_.text > elem.text, right)
    )
  }

  def mostRetweeted: Tweet = {
    def getOrNil: PartialFunction[TweetSet, Tweet] = {
      case _: Empty => NilTweet
      case a: NonEmpty => a.mostRetweeted
    }
    Seq(getOrNil(left), getOrNil(right), elem).max(new Ordering[Tweet] {
      override def compare(x: Tweet, y: Tweet): Int = x.retweets.compareTo(y.retweets)
    })
  }
